$execute if data storage item_border:database collected[{id: "$(id)"}] if data storage item_border:codex index[{id:"$(id)"}] as @s run return fail
$execute unless data storage item_border:codex index[{id:"$(id)"}] as @s run return fail
